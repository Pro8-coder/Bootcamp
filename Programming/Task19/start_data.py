...
def new_generation():
    ...
    s=seek_max() #находим чемпиона и его знчение ЦФ (находим максимальную прибыль в прошлом поколении)
    pokolenie=[]
    for i in range(childs): #оставляем все особи у которых целевая функция больше чем 70% от максимального значения
...
def fill_out():    #заполнение таблицы
    ...
    out.append([str(round(sum_marja,2)),"",str(round(sum_cost,2)),"ВСЕГО ОДОБРЕНО "+str(count_projects)+" ПРОЕКТОВ , ЛИМИТ ДЕНЕГ РАВЕН "+str(up_cost),"",""])
fill_out()
